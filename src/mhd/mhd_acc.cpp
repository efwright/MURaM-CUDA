...
//-------------------------------------------------------------
double mhd_openacc(
  cState *U, cState *R, Vector *curlB, double *BgradT, Vector *curlBxB,
  double *pres, double *sflx, double *rhoi, Vector *amb,
  Vector *v_amb, double *tvar1, double *tvar2, double *tvar3,
  double *Qamb, double *Rflx, Vector *R_amb, double *temp,
  GridData &Grid
) {
  ...
  const bool ambipolar        = false;
  const bool spitzer          = true;
  const bool needs_curlB      = (eta > 0.0);
  const bool need_diagnostics = false;
  
  const int nvar   = Grid.nvar;
  const int vsize  = Grid.vsize;
  const int v_nvar = Grid.nvar;
  ...
  const double ambfac_max = 0.0; //Physics.params[i_param_ambfac_max];
 
  const int lsize[3] = {nz, nx, ny};
  const int ghosts[3] = {2, 2, 2}; 

  const int lbeg[3] = {Grid.lbeg[0], Grid.lbeg[1], Grid.lbeg[2]};
  const int lend[3] = {Grid.lend[0], Grid.lend[1], Grid.lend[2]};
  ...
  if(needs_curlB) {
#pragma acc parallel loop present(curlB[:bufsize]) 
    for(i=0; i<bufsize; i++) {
      curlB[i].x = 0;
      curlB[i].y = 0;
      curlB[i].z = 0;
  ...
  if(spitzer) {
#pragma acc parallel loop present(BgradT[:bufsize])
    for(i=0; i<bufsize; i++)
      BgradT[i] = 0;
  }

  if(ambipolar) {
#pragma acc parallel loop present(curlBxB[:bufsize])
    for(i=0; i<bufsize; i++) {
      curlBxB[i].x = 0;
      curlBxB[i].y = 0;
      curlBxB[i].z = 0;
...
*/
  ...
  amb_diff_max = 0.0;
  amb_vel_max  = 0.0;
#pragma acc data  present(U[:bufsize], R[:bufsize],                 \
   pres[:bufsize],   sflx[:bufsize],               \
   rhoi[:bufsize],   amb[:bufsize],                \
   v_amb[:bufsize],  tvar1[:bufsize],              \
   tvar2[:bufsize],  tvar3[:bufsize],              \
   Qamb[:bufsize],   curlBxB[:bufsize],            \
   BgradT[:bufsize], Rflx[:bufsize],               \
   curlB[:bufsize],  R_amb[:bufsize],              \
   temp[:bufsize]) 
{
...
#pragma acc loop vector private(node) 
  ...
  PCAST(pcast::kernel_output({pcast::U, pcast::Res, pcast::pres, pcast::sflx, pcast::rhoi, \
     pcast::amb, pcast::v_amb, pcast::tvar1, pcast::tvar2, pcast::tvar3, \
     pcast::Qamb, pcast::curlBxB, pcast::BgradT, pcast::Rflx, \
     pcast::curlB, pcast::R_amb, pcast::temp}));
