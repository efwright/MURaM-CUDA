...
/*
__global__ void integrate_kernel_graphs(
...
)*/
{
  ...
  if(!graphCreated) {
    ...
    for(int i = 0; i < bound0; i++) {
      integrate_kernel_graphs<<<blocks, threads, 0, stream>>>(
        //I_n, coeff, nx, ny, nz, c0, c1, c2, c3, off0, off1, off2, off3, y
        I_n, coeff, n,
  ...
  CHECK_CUDA_ERROR(cudaGraphLaunch(instance, 0));
  CHECK_CUDA_ERROR(cudaStreamSynchronize(0));
}
